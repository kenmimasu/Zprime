      REAL*8 FUNCTION SBBB_TTB(JF,MASSF,LAM1,LAM2,P1, P2, P3, P4)
C  
...
C FOR PROCESS : b b~  -> t/b t~/b~ (via  gluon) 
C  
      IMPLICIT NONE
C  
C CONSTANTS
...
C          IF (GOODHEL(IHEL) .OR. NTRY .LT. 10) THEN
             T=BBB_TTB(TB,MASSF,LAM1,LAM2,P1, P2, P3, P4,NHEL(1,IHEL)) 
             SBBB_TTB = SBBB_TTB + T
...
C          ENDIF
      ENDDO
      SBBB_TTB = SBBB_TTB /  4D0 
      END
       
       
      REAL*8 FUNCTION BBB_TTB(TB,MASSF,LAM1,LAM2,P1, P2, P3, P4,NHEL)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT IN PHASE SPACE P1,P2,P3,P4,...
...
C  
C FOR PROCESS : b b~  -> b/t b/t~ (via gluon) 
C  
      IMPLICIT NONE
C  
C CONSTANTS
...
c      CALL IXXXXX(P1  ,ZERO,NHEL(1  ), 1,W1  )                       
c      CALL OXXXXX(P2  ,ZERO,NHEL(2  ),-1,W2  )                       
...
c S-CHANNEL GLUON                     
      ...
      IF(TB.EQ.1)THEN
c T-CHANNEL GLUON                     
      CALL JIOXXX(W1  ,W3  ,GG,ZERO,ZERO,W6  )                             
      CALL IOVXXX(W4  ,W2  ,W6  ,GG,AMP(2  ))  
      ENDIF                       
      BBB_TTB = 0.D0 
      DO I = 1, NEIGEN
          ZTEMP = (0.D0,0.D0)
          ...
          ENDDO
          BBB_TTB =BBB_TTB+ZTEMP*EIGEN_VAL(I)*CONJG(ZTEMP) 
      ENDDO
