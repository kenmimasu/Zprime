...
c successive pair of indices correspond to ith level KK photon followed by KK Z
      Rinv = param(1)
      do i=1,nzp/2
            rmZZ(2*i-1)=gp(i)*float(i)*Rinv
c            rmZZ(2*i)=gp(i)*sqrt(rmZ*rmZ + (float(i)*Rinv)**2.d0 )
            ggvu(2*i-1)=gp(i)*sqrt(2.d0)*(gau(1)+gau(2))/2.d0
            ggau(2*i-1)=gp(i)*sqrt(2.d0)*(gau(1)-gau(2))/2.d0
c            ggvu(2*i)=gp(i)*sqrt(2.d0)*(gzu(1)+gzu(2))/2.d0
c            ggau(2*i)=gp(i)*sqrt(2.d0)*(gzu(1)-gzu(2))/2.d0
            ggvd(2*i-1)=gp(i)*sqrt(2.d0)*(gad(1)+gad(2))/2.d0
            ggad(2*i-1)=gp(i)*sqrt(2.d0)*(gad(1)-gad(2))/2.d0
c            ggvd(2*i)=gp(i)*sqrt(2.d0)*(gzd(1)+gzd(2))/2.d0
c            ggad(2*i)=gp(i)*sqrt(2.d0)*(gzd(1)-gzd(2))/2.d0
      end do
