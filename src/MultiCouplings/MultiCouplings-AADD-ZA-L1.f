...
c custom rmZZ assignments for AADD where param = R^-1
c custom VA coupling assignments for quarks in AADD (sqrt(2)*SM) 
c ONLY INCLUDES FIRST KK LEVEL!!!
       Rinv=param(1)
c      do i=1,nzp/2
            rmZZ(1)=gp(1)*Rinv
            rmZZ(2)=gp(1)*sqrt(rmZ*rmZ + (Rinv)**2.d0 )
            ggvu(1)=gp(1)*sqrt(2.d0)*(gau(1)+gau(2))/2.d0
            ggau(1)=gp(1)*sqrt(2.d0)*(gau(1)-gau(2))/2.d0
            ggvu(2)=gp(1)*sqrt(2.d0)*(gzu(1)+gzu(2))/2.d0
            ggau(2)=gp(1)*sqrt(2.d0)*(gzu(1)-gzu(2))/2.d0
            ggvd(1)=gp(1)*sqrt(2.d0)*(gad(1)+gad(2))/2.d0
            ggad(1)=gp(1)*sqrt(2.d0)*(gad(1)-gad(2))/2.d0
            ggvd(2)=gp(1)*sqrt(2.d0)*(gzd(1)+gzd(2))/2.d0
            ggad(2)=gp(1)*sqrt(2.d0)*(gzd(1)-gzd(2))/2.d0
c      end do
