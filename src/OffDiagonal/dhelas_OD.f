...
c NOTE: hacked to allow complex masses/widths compatible with off diagonal
c width implementation.
...
c
      ...
      complex*16   vmass,vwidth,vm2
      real*8    g(2),q(0:3),q2,dd
      real*8 revm2,imvm2
c      real*8    g(2),q(0:3),vmass,vwidth,q2,vm2,dd
c
      real*8 r_zero, r_one
      parameter( r_zero=0.0d0, r_one=1.0d0 )
      complex*16 c_imag
...
C       vm2=vmass**2
      ...
      imvm2= imag(vm2)
c
C       if (vmass.ne.r_zero) then
       if (abs(vm2).ne.r_zero) then
c
c         d=r_one/dcmplx( q2-vm2 , max(sign( vmass*vwidth ,q2),r_zero) )
      d=r_one/dcmplx( q2-revm2 , 
     &          -imvm2+max(sign( abs(vmass*vwidth) ,q2),r_zero))
