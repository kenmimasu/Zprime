...
c few extra variables for looping over M_s, calculating CL, invariant mass cut
     ...
     &          resbbalt(-1:1,-1:1)
      ...
      real*8 pfx(-1:1,-1:1)
      real*8 pfxalt(-1:1,-1:1)
      real*8 weight(20,-1:1,-1:1) 
...
c x1 and x2 of the partons.
      ...
      ex(2,2)=xx1     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
C start loop over (x1,x2) and (x2,x1)
      do ix=1,2
      x1 = ex(ix,1)
      x2 = ex(ix,2)
c symmetry around ct.
      ct = x(3) ! send ct to -ct for alternate x configuration
c      ct = (3-2*ix)*x(3) ! send ct to -ct for alternate x configuration
      st=sqrt(1.d0-ct*ct)
c initialise
      fxn1=0.d0
      fxn2=0.d0
...
c construct polarised hadronic structure functions.
      call setpdf(x1, x2, QQ, istructure, icoll, *626, ffxn, fx1, fx2)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
C       print*, fxn1,fxn2,ffxn
      fxn1 = fxn1/2d0 ! account for summing over both x configs.
      fxn2 = fxn2/2d0
      ffxn = fxn1+fxn2
cccccccccccccccccccccc
C histogram weights
      hist1=fxn1*wgt ! weight for q-qbar initial state: costheta(CM)=ct
      hist2=fxn2*wgt ! weight for qbar-q initial state: costheta(CM)=-ct
      hist=hist1+hist2 ! total weight for insensitive observables 
cccccccccccccccccccccc
